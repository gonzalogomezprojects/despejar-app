...
// Explicita que es dinámico en Next 15
export const dynamic = "force-dynamic";
export const revalidate = 0;

/* -------- Configuración de rate limit -------- */
const RATE_LIMIT = Number(process.env.RATE_LIMIT ?? 60); // peticiones por ventana
const WINDOW_MS = Number(process.env.RATE_WINDOW_MS ?? 60_000); // duración ventana en ms

type Bucket = { count: number; reset: number };
type Store = Map<string, Bucket>;

// Guardamos el store en memoria del proceso (funciona en instancias "calientes")
...
function rateHeaders(limit: number, remaining: number, resetMs: number) {
  return {
    ...
    "X-RateLimit-Reset": String(Math.floor(resetMs / 1000)), // epoch seconds
  };
}
...
function applyRateLimit(key: string) {
  ...
  // limpieza ligera de buckets expirados
...
export async function GET(req: Request) {
  ...
  try {
    const body = pickRandom(phrases);
    return new NextResponse(JSON.stringify(body), {
      status: 200,
      headers: baseHeaders,
    });
  } catch {
    return new NextResponse(JSON.stringify({ error: "Error al leer frases" }), {
      status: 500,
      headers: baseHeaders,
    });
  }
